...
3. HOW TO RUN IT
   EXE (recommended) - ONE-FOLDER app:
     a. GAMGUI ships as a FOLDER (GAMGUI\ containing GAMGUI.exe and an
        _internal\ support folder). Keep the folder together - do not move
        GAMGUI.exe out of it. Recommended location: C:\GAM7\GAMGUI\.
     b. Launch C:\GAM7\GAMGUI\GAMGUI.exe, or use the "Launch GAMGUI"
        shortcut placed in C:\GAM7.
     c. gam.exe is found automatically if it is on the PATH (it is, in the
        standard C:\GAM7 install). If not, click "Locate gam.exe..." (top
        right); the choice is remembered in gamgui.ini.
     NOTE: it is a folder, not a single .exe, on purpose - the single-file
     build crashed intermittently on Python 3.14 (Tcl/Tk 9). See CHANGELOG
     v1.9. Pin/shortcut GAMGUI.exe inside the folder for easy access.
   ...
   Building the app yourself (Windows):
     ...
     b. Run Build-EXE.bat. It first runs extract_tcl.py (which pulls the
        ...
        Python 3.14, or the build crashes with "_tcl_data not found"),
        then builds the one-folder app with those libraries bundled.
     c. The result is dist\GAMGUI\ - copy the whole folder to its home.
     For macOS/Linux: build on that OS with
        pyinstaller --onedir --windowed --name GAMGUI GAMGUI.py
     (the Tcl zipfs issue is Python-3.14-specific; if you hit a missing
     Tcl data error there, apply the same extract_tcl.py + --add-data
     approach shown in Build-EXE.bat).
...
8. KNOWN LIMITATIONS
   ...
   - The incident-response workflow pulls raw Gmail/Drive audit CSVs but
     ...
     persisted attachments (the original batch workflow's deepest
